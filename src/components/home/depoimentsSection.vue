<template>
    <div class="grid grid-cols-5 grid-rows-5 bg-[#E6007E] p-[7vw] gap-[7vw] relative">
      <div @click="prevTestimonial" class="absolute left-[2vw] top-[50%] w-[3em] h-[3em] rounded-full cursor-pointer hover:scale-[1.05] duration-200 ease-in-out">
        <img src="/assets/icons/arrow_circle.png" alt="Previous">
      </div>
      <div @click="nextTestimonial" class="absolute right-[2vw] top-[50%] w-[3em] h-[3em] rounded-full cursor-pointer hover:scale-[1.05] duration-200 ease-in-out">
        <img src="/assets/icons/arrow_circle.png" alt="Next" class="rotate-180">
      </div>
      
      <span class="absolute left-[7vw] top-[14vh] font-bold text-[40px] text-center text-white">{{ $t('home.depoiments.title') }}</span>
      
      <transition name="fade" mode="out-in">
        <div :key="currentIndex" class="col-span-3 row-span-5 pt-[5em] text-white flex flex-col h-auto justify-between ">
          <div class="h-8"></div>
          <div class="flex gap-4 ">
            <div class="rounded overflow-hidden">
              <img :src="currentTestimonial.picture" alt="Author" class="w-[5em] h-[5em] object-cover">
            </div>
          ...
          <span class="text-[20px] w-full min-h-[8em]">{{ currentTestimonial.content }}</span>
          <div class=h-8></div>
        </div>
      </transition>
  
      <div class="grid col-span-4 row-span-5 col-start-4 grid-cols-9 grid-rows-10 gap-4 h-auto">
        <div class="col-span-9 row-span-10 bg-[white] w-[40em] h-[30em] rounded overflow-hidden">
            <img :src="currentTestimonial.img" alt="" class=" w-full h-full object-cover">
        </div>
      </div>
    </div>
  </template>
  ...
  export default {
      ...
      return {
        ...
        depoiments: [
          ...
          },
          {
            ...
            content: 'Nullam quis risus eget urna mollis ornare vel eu leo. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Nullam id dolor id nibh ultricies vehicula ut id elit. Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor.',
    ...
    methods: {
      ...
      prevTestimonial() {
        this.currentIndex = (this.currentIndex - 1 + this.depoiments.length) % this.depoiments.length;
      }
    }
  }
  </script>
